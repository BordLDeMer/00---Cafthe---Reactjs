...
function Footer() {
    ...
    const fetchProduits = async () => {
      try {
        const response = await axios.get(
          `${process.env.REACT_APP_API_URL}/api/products/solde`,
        );
  ...
  const settings = {
    ...
    slidesToScroll: 6, // Nombre de produits défilés par scroll
    ...
    responsive: [
      ...
      },
      {
        ...
        settings: {
          slidesToShow: 2, // Afficher 3 produits
          slidesToScroll: 1, // Faire défiler 1 produit à la fois
        },
      },
    ],
  ...
  return (
    <div className="Footer">
      <h1>Profitez de nos promotions du moment</h1>
      <div>
        <Slider {...settings}>
          {produits.map((produit) => (
            <div key={produit.id}>
